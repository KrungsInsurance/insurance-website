...
export async function handleChat(request:Request,options:{offlineDemo?:boolean}={}){
 ...
 const addCard=(value:unknown)=>{const card=chatCardSchema.parse(value);if(conversationalOnly&&card.type!=="term")throw new Error("information_only");if("planIds" in card){const ps=checkedPlans(card.planIds);if(new Set(ps.map(p=>p.category)).size!==1)throw new Error("category");if("fieldKeys" in card&&(new Set(card.fieldKeys).size!==card.fieldKeys.length||card.fieldKeys.some(k=>!categoryFields[ps[0].category].some(f=>f.key===k))))throw new Error("field");}
