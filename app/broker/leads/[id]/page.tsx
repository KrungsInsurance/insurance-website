...
import "@/components/broker-detail-readability.css";
...
function BrokerLeadWorkspace({id}:{id:string}) {
  ...
  if (!lead) return <main className="broker-workspace broker-lead-page"><div className="broker-container broker-empty"><MessageSquare size={32} aria-hidden="true"/><h1>ไม่พบคำขอ</h1><p>ลิงก์นี้ไม่มีคำขอในข้อมูลเดโมของเบราว์เซอร์นี้</p><Link href="/broker" className="broker-secondary">กลับรายการลูกค้า</Link></div></main>;
  ...
  return <main className="broker-workspace broker-lead-page"><div className="broker-demo-strip">โหมดผู้เชี่ยวชาญ · ChatGPT จำลองลูกค้า · ไม่มีการติดต่อบุคคลจริง</div><div className="broker-container">
   <Link href="/broker" className="broker-back"><ArrowLeft size={16} aria-hidden="true"/>กลับรายการลูกค้า</Link>
   <header className="broker-detail-heading"><div><p className="broker-eyebrow">ข้อมูลลูกค้า</p><h1>{lead.displayName}</h1><p>ส่งเมื่อ <time dateTime={lead.createdAt}>{new Date(lead.createdAt).toLocaleString("th-TH",{timeZone:"Asia/Bangkok"})}</time></p></div><span className={`broker-status broker-status-${lead.status}`} role="status">{statusLabel}</span></header>
   ...
   <div className="broker-work-grid"><div className="broker-communication">
    ...
    <details className="broker-panel broker-disclosure"><summary>สรุปฉบับเต็ม</summary>{lead.summary.overview?<AssistantOverviewView overview={lead.summary.overview} plans={lead.planFacts}/>:<p className="broker-muted">คำขอเก่ายังไม่มีข้อมูลสรุปจากแชต</p>}</details>
    <details className="broker-panel broker-disclosure"><summary>แผน ณ ตอนส่งต่อ · {lead.planFacts?.length??0} แผน</summary>{lead.planFacts?.length?lead.planFacts.map(plan=><div key={plan.id} className="broker-saved-plan"><h3>{plan.name}</h3><p>{plan.price?formatPrice(plan.price):formatPremium(plan.premiumTHB,plan.premiumPeriod,plan.premiumNote)}</p><HistoricalCard card={{type:"plans",planIds:[plan.id],fieldKeys:[]}} lead={lead}/></div>):<p className="broker-muted">คำขอเก่าไม่มีข้อมูลแผนที่บันทึกไว้</p>}</details>
</aside></div>
  </div></main>;
}
