...
export default function BrowsePage() {
  ...
  const plans = useMemo(() => searchPlans({ category, q, maxPremium: budgetForSearch, sort }), [budgetForSearch, category, q, sort]);
  const quotePlans = useMemo(() => budgetForSearch === undefined ? [] : searchPlans({category,q,sort}).filter(plan => plan.price.kind === "quote_only"), [budgetForSearch,category,q,sort]);
  ...
  <section className="catalog-surface"><div className="catalog-content"><header className="catalog-heading"><div><h2>รู้จักประกัน{categoryLabels[category]}.</h2><p>ข้อมูลจากบริษัทประกัน · ตรวจเงื่อนไขก่อนตัดสินใจ</p></div><p role="status">{plans.length} แผนที่พบ</p></header>
  ...
  {plans.length?<div key={`results-${searchParams.toString()}`} className="catalog-grid catalog-results-enter">{renderPlans(plans)}</div>:<div key={`empty-${searchParams.toString()}`} className="catalog-empty catalog-results-enter"><h3>ยังไม่พบแผนที่ตรงกับตัวกรอง</h3><p>ลองขยายงบ หรือล้างคำค้นหาเพื่อดูแผนในหมวดนี้</p><Link href={href({q:"",maxPremium:""})} onClick={()=>flushSync(()=>setProfile({...state.profile,budgetTHB:null}))} className="text-link">ล้างตัวกรอง ›</Link></div>}
  {quotePlans.length>0&&<section className="catalog-quotes"><h2>แผนที่ต้องขอราคา.</h2><p>ยังยืนยันไม่ได้ว่าอยู่ในงบที่เลือก จึงแสดงแยกจากผลลัพธ์ด้านบน</p><div key={`quotes-${searchParams.toString()}`} className="catalog-grid catalog-results-enter">{renderPlans(quotePlans)}</div></section>}
