...
export default function BrowsePage() {
  ...
  {plans.length?<div className="catalog-grid">{renderPlans(plans)}</div>:<div className="catalog-empty"><h3>ยังไม่พบแผนที่ตรงกับตัวกรอง</h3><p>ลองขยายงบ หรือล้างคำค้นหาเพื่อดูแผนในหมวดนี้</p><Link href={href({q:"",maxPremium:""})} onClick={()=>flushSync(()=>setProfile({...state.profile,budgetTHB:null}))} className="text-link">ล้างตัวกรอง ›</Link></div>}
  {quotePlans.length>0&&<section className="catalog-quotes"><h2>แผนที่ต้องขอราคา.</h2><p>ยังยืนยันไม่ได้ว่าอยู่ในงบที่เลือก จึงแสดงแยกจากผลลัพธ์ด้านบน</p><div className="catalog-grid">{renderPlans(quotePlans)}</div></section>}
