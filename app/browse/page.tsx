...
const categoryLabels: Record<Category, string> = { health: "สุขภาพ", motor: "รถยนต์", life: "ชีวิต", accident: "อุบัติเหตุ", travel: "เดินทาง", property: "บ้านและทรัพย์สิน", liability: "ความรับผิด" };
...
export default function BrowsePage() {
  ...
  const plans = useMemo(() => searchPlans({ category, q, sort }).filter(plan => plan.price.kind === "quote_only" || budgetForSearch === undefined || (plan.price.amountTHB !== null && plan.price.amountTHB <= budgetForSearch)), [budgetForSearch, category, q, sort]);
  ...
  function href(next: Partial<{ category: Category; q: string; maxPremium: string; sort: PlanSort }>) {
  ...
  function renderPlans(items: Plan[]) { return items.map(plan => <article key={plan.id} className="catalog-card"><div className="catalog-art" data-image-kind={planImages[plan.id]?.kind}><img src={plan.image} alt={planImages[plan.id]?.alt??plan.name} loading="lazy"/>{planImages[plan.id]?.kind==="insurer"&&<span className="catalog-image-caption">ภาพบริษัทประกัน</span>}</div><div className="catalog-card-body"><p className="catalog-company">{plan.insurer}</p><h3>{plan.name}</h3><PlanPrice price={plan.price} className="catalog-price"/><PlanTradeoffs plan={plan}/><div className="catalog-card-actions"><Link href={`/plans/${plan.id}`} className="action-primary">ดูรายละเอียด</Link><label><input type="checkbox" checked={selectedIds.includes(plan.id)} onChange={()=>togglePlan(plan.id)}/>เปรียบเทียบ</label></div>{selectionError?.id===plan.id&&<p role="alert" className="text-red-700 text-sm mt-2">{selectionError.message}</p>}<details className="catalog-extra"><summary>ความคุ้มครองเด่น</summary><ul>{plan.highlights.map(item=><li key={item}>{item}</li>)}</ul><a href={planImages[plan.id]?.sourceUrl} target="_blank" rel="noopener noreferrer" className="catalog-image-source">ที่มาของภาพจากบริษัท ↗</a></details></div></article>); }
  const insurers=[...new Set(searchPlans({category}).map(plan=>plan.insurer))];
  return <main className="browse-page"><section className="catalog-intro"><header><h1>ประกัน</h1><p>เลือกความคุ้มครอง<br/>ที่เข้ากับชีวิตคุณ.</p></header><nav className="category-rail" aria-label="เลือกหมวดประกัน">{categories.map(item=>{const Icon=categoryIcons[item];return <Link key={item} href={href({category:item,q:""})} onClick={event=>{event.preventDefault();categoryTrigger.current=event.currentTarget;changeCategory(item);}} aria-current={item===category?"page":undefined}><Icon size={36} strokeWidth={1.35}/><span>{categoryLabels[item]}</span></Link>;})}</nav></section>
  <section className="catalog-surface"><div className="catalog-content"><header className="catalog-heading"><div><h2>รู้จักประกัน{categoryLabels[category]}.</h2><p>ข้อมูลจากบริษัทประกัน · ตรวจเงื่อนไขก่อนตัดสินใจ</p></div><p role="status">{plans.length} แผนที่พบ{quotePlans.length>0&&` · ขอใบเสนอราคา ${quotePlans.length} แผน`}</p></header>
  ...
  <details className="catalog-filters" open={Boolean(q||rawBudget||budgetError)}><summary>ค้นหาและกรองเพิ่มเติม{q||rawBudget?" · มีตัวกรอง":""}</summary><div><form key={`search-${searchParams.toString()}`} action={submitSearch}><label htmlFor="plan-search">ค้นหาแผนหรือบริษัท</label><div className="catalog-input-row"><input id="plan-search" name="q" className="field" defaultValue={q} placeholder="ชื่อแผน หรือความคุ้มครอง"/><button className="action-secondary">ค้นหา</button></div></form><form key={`budget-${searchParams.toString()}`} action={submitBudget}><label htmlFor="max-premium">งบสูงสุด ({periodLabel}) · ไม่จำเป็นต้องระบุ</label><div className="catalog-input-row"><input id="max-premium" name="maxPremium" className="field" inputMode="decimal" defaultValue={rawBudget??""} placeholder="ไม่จำกัด" aria-invalid={budgetError} aria-describedby={budgetError?"budget-error":undefined}/><button className="action-secondary">ใช้</button></div>{budgetError&&<p id="budget-error" role="alert" className="text-red-700">กรอกงบเป็นตัวเลขตั้งแต่ 0 ขึ้นไป</p>}</form><label>เรียงตาม<select className="field" aria-label="เรียงผลลัพธ์" value={sort} onChange={event=>router.push(href({sort:event.target.value as PlanSort}))}>{(Object.keys(sortLabels) as PlanSort[]).map(item=><option key={item} value={item}>{sortLabels[item]}</option>)}</select></label></div></details>
  {(categoryChanged||sortChanged)&&<p className="catalog-notice">ใช้ค่าเริ่มต้นสำหรับตัวกรองที่ไม่รู้จัก</p>}
  {selectedIds.length>0&&<aside className="catalog-selection" aria-live="polite"><p>เลือกไว้ {selectedIds.length}/3 แผน <span>{selectedIds.map(id=>getPlan(id)?.name).join(" · ")}</span>{selectedIds.length===1&&<span>เลือกเพิ่มอีก 1 แผนเพื่อเปรียบเทียบ</span>}</p><div><button onClick={()=>setSelectedIds([])} className="text-link"><X size={16} aria-hidden="true"/>ล้าง</button>{selectedIds.length>=2&&<Link href={`/compare?category=${getPlan(selectedIds[0])?.category??category}&ids=${selectedIds.join(",")}`} className="action-primary">เปรียบเทียบ <ArrowRight size={16} aria-hidden="true"/></Link>}</div></aside>}

  {quotePlans.length>0&&<p className="catalog-notice">{budgetForSearch===undefined?"รวมแผนที่ต้องขอใบเสนอราคา ดูจุดเด่นและข้อจำกัดก่อนได้โดยยังไม่ต้องกำหนดงบ":"รวมแผนที่ต้องขอใบเสนอราคาไว้ให้พิจารณาด้วย แต่ยังยืนยันไม่ได้ว่าอยู่ในงบที่เลือก"}</p>}
  {plans.length>0?<div key={`results-${searchParams.toString()}`} className="catalog-grid catalog-results-enter">{renderPlans(plans)}</div>:<div key={`empty-${searchParams.toString()}`} className="catalog-empty catalog-results-enter"><h3>ยังไม่พบแผนที่ตรงกับตัวกรอง</h3><p>ลองขยายงบ หรือล้างคำค้นหาเพื่อดูแผนในหมวดนี้</p><Link href={href({q:"",maxPremium:""})} onClick={()=>flushSync(()=>setProfile({...state.profile,budgetTHB:null}))} className="text-link">ล้างตัวกรอง ›</Link></div>}
  <p className="catalog-footnote">ราคาและความคุ้มครองอ้างอิงตามแผนและเงื่อนไขที่ระบุ ไม่ใช่ใบเสนอราคาเฉพาะบุคคล · เว็บไซต์เดโม ไม่มีการซื้อหรือออกกรมธรรม์จริง</p></div></section><AlertDialog.Root open={pendingCategory !== null} onOpenChange={open => { if (!open) { setPendingCategory(null); setPendingPlan(null); } }}><AlertDialog.Portal><AlertDialog.Overlay className="fixed inset-0 z-40 bg-black/30"/><AlertDialog.Content onCloseAutoFocus={event => { event.preventDefault(); categoryTrigger.current?.focus(); }} className="fixed left-1/2 top-1/2 z-50 w-[min(420px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6"><AlertDialog.Title className="text-xl font-semibold">{pendingPlan ? "เปลี่ยนกลุ่มแผนที่เปรียบเทียบ?" : "เปลี่ยนหมวดประกัน?"}</AlertDialog.Title><AlertDialog.Description className="mt-3 text-sm">{pendingPlan ? "แผนนี้เปรียบเทียบร่วมกับรายการเดิมไม่ได้ ยืนยันเพื่อล้างรายการเดิมและเลือกแผนนี้" : "แผนที่เลือกไว้จะถูกล้าง เพื่อเปรียบเทียบเฉพาะหมวดเดียวกัน"}</AlertDialog.Description><div className="mt-6 flex gap-3"><AlertDialog.Cancel className="action-secondary">ยกเลิก</AlertDialog.Cancel><AlertDialog.Action className="action-primary" onClick={() => { if (!pendingCategory) return; flushSync(() => { setSelection(pendingPlan ? [pendingPlan] : []); setProfile({ ...state.profile, preferredCategory: pendingCategory }); }); window.location.assign(href({ category: pendingCategory, q: "" })); }}>{pendingPlan ? "เปลี่ยนกลุ่มแผน" : "เปลี่ยนหมวด"}</AlertDialog.Action></div></AlertDialog.Content></AlertDialog.Portal></AlertDialog.Root></main>;
}
