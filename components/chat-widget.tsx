...
import { personaBudgetLabel, personaOpening, personaFieldKeys, type DiscoveryPersona } from "@/lib/discovery-persona";
...
export function ChatWorkspace({fullPage=false}:{fullPage?:boolean}){
 ...
 const workspace=<>
  <header className="chat-header"><div className="chat-header-title">{fullPage&&<Link href="/" className="chat-home-link" aria-label="กลับหน้าแรก"><ArrowLeft size={22} aria-hidden="true"/></Link>}<div>{fullPage?<h1>คุยเรื่องประกัน</h1>:<Dialog.Title>คุยเรื่องประกัน</Dialog.Title>}{!fullPage&&<Dialog.Description className="sr-only">ถามเรื่องประกัน</Dialog.Description>}</div></div>
  ...
  <div className="chat-scroll" ref={scrollArea}><div className="chat-reading-column"><p className="sr-only" role="status" aria-live="polite" aria-atomic="true">{announcement}</p>
   <div className="chat-messages">{state.messages.map((message,i)=><div key={i} ref={i===latestAssistant?lastReply:undefined} className={"chat-message "+(message.role==="user"?"is-user":"is-assistant")}><p className={"chat-bubble "+(message.role==="user"?"is-user":"")}>{message.content}</p>{message.role==="assistant"&&message.cards?.map((card,j)=><ChatCardView key={j} card={card} selection={state.selection} disabled={busy} current={i===latestAssistant&&!onboarding} onAction={cardAction} onCategory={chooseCategory} onBudget={chooseBudget} onIntake={text=>void send(text)}/>)}</div>)}
