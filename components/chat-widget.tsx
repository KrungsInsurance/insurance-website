...
import { interestScore, makeSummary, chatRequestBody } from "@/lib/demo-state";
...
const responseSchema=z.object({message:z.string().min(1).max(500),mode:z.enum(["live","mock"]),cards:chatCardsSchema});
...
export function ChatWorkspace({fullPage=false}:{fullPage?:boolean}){
 ...
 const {state,setSelection,setInterestedPlans,setProfile,appendMessages,addLead,saveSummary,recordSignal,recordComparison}=useDemo();
 ...
 const [mode,setMode]=useState<"live"|"mock">(draft?.mode??"live"),[lastMode,setLastMode]=useState<"live"|"mock"|null>(state.messages.filter(message=>message.role==="assistant").at(-1)?.mode??null);
 ...
 const score=interestScore(state.signals);
 ...
 function chooseCategory(category:Category,confirmed=false){
  ...
  const profile={...state.profile,preferredCategory:category,budgetTHB:fullPage?null:state.profile.budgetTHB},selection=state.selection.filter(id=>getPlan(id)?.category===category);
  ...
  if(fullPage){setStep("budget");setError("");return;}
  void send(`สนใจประกัน${labels[category]} ช่วยเริ่มเลือกแผน ถามทีละข้อและให้เลือกตอบในแชต`,{profile,selection});
 }
 ...
 async function send(content=input,nextContext?:{profile:CustomerProfile;selection:string[]}){
  ...
  try{
   const response=await fetch(`/api/chat${mode==="mock"?"?mode=mock":""}`,{method:"POST",headers:{"content-type":"application/json"},body:chatRequestBody(messages,{category:profile.preferredCategory,selectedPlanIds:selection,budgetTHB:profile.budgetTHB,goals:profile.goals})});
 ...
 const workspace=<>
  <header className="chat-header"><div className="chat-header-title">{fullPage&&<Link href="/" className="chat-home-link" aria-label="กลับหน้าแรก"><ArrowLeft size={22} aria-hidden="true"/></Link>}<div>{fullPage?<h1>ผู้ช่วยเลือกประกัน</h1>:<Dialog.Title>ผู้ช่วยเลือกประกัน</Dialog.Title>}{fullPage?<p>ถามให้เข้าใจ แล้วค่อยตัดสินใจ</p>:<Dialog.Description>{mode==="mock"?"บทสนทนาตัวอย่าง":"Live AI · ส่งข้อความไป OpenAI"}</Dialog.Description>}</div></div>
