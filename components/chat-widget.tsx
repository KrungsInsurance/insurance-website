"use client";
import { MessageCircle, Send, X, Minus, Expand, Shrink, ArrowLeft, ArrowUpRight } from "lucide-react";
import { usePathname, useSearchParams } from "next/navigation";
import Link from "./native-link";
import { flushSync } from "react-dom";
import { DiscoveryOnboarding } from "./discovery-onboarding";
import { ageBands, personaBudgetLabel, personaOpening, personaFieldKeys, type DiscoveryPersona } from "@/lib/discovery-persona";
...
import { CHAT_HISTORY_LIMIT } from "@/lib/chat-memory";
import { makeSummary, chatRequestBody, chatContinuation, startDiscoveryState } from "@/lib/demo-state";
...
export function ChatWidget(){const pathname=usePathname();return pathname==="/chat"||pathname==="/broker"||pathname.startsWith("/broker/")?null:<ChatWorkspace/>;}
export function ChatWorkspace({fullPage=false}:{fullPage?:boolean}){
 const params=useSearchParams();
 const [draft]=useState(readDraft);
 const {state,startDiscovery,setAssistantOverview,setSelection,setInterestedPlans,setProfile,appendMessages,addLead,saveSummary,recordSignal,recordComparison}=useDemo();
 ...
 const [lastMode,setLastMode]=useState<"live"|"mock"|null>(state.messages.filter(message=>message.role==="assistant").at(-1)?.mode??null);
 ...
 const [step,setStep]=useState<"persona"|null>(fullPage&&params.get("handoff")!=="1"&&(params.get("start")==="1"||!state.messages.length)?"persona":null);
 ...
 useEffect(()=>{
  const fn=(event:Event)=>{
   ...
   setOpen(true);setMinimized(false);setUnread(false);setOnboarding(Boolean(detail?.start));setHandoff(Boolean(detail?.handoff));setConsent(false);setSubmitted("");setSummaryErrors({});
   if(detail?.start){setError("");setInput("");}
   if(typeof detail?.draft==="string"&&categories.includes(detail.category)){
    if(lock.current){setError("รอคำตอบก่อนนำข้อมูลใหม่เข้ามาในแชต");return;}
    // The shared composer is single-line; keep field boundaries when editing a multi-line draft.
    const text=detail.draft.trim().replace(/[\r\n]+/g,"; "),category=detail.category as Category;
    if(!text||text.length>2000){setError("ย่อข้อมูลให้ไม่เกิน 2,000 ตัวอักษร แล้วเปิดแชตอีกครั้ง");return;}
    const applyDraft=()=>{
     const changed=state.profile.preferredCategory!==category;
     const profile={...state.profile,preferredCategory:category,...(changed?{budgetTHB:null,goals:[]}:{} )};
     const selection=state.selection.filter(id=>getPlan(id)?.category===category);
     setProfile(profile);setSelection(selection);contextRef.current=JSON.stringify([profile,selection]);
     setStep(null);setInput(text);setError("");setOnboarding(false);
     requestAnimationFrame(()=>composer.current?.focus());
    };
    if([...state.selection,...state.interestedPlanIds].some(id=>getPlan(id)?.category!==category)){
     actionTrigger.current=document.activeElement as HTMLElement;
     setConfirmation({description:`แผนที่เลือกจากหมวดเดิมจะถูกล้าง เพื่อคุยเรื่องประกัน${labels[category]}`,confirm:applyDraft});
    }else applyDraft();
   }
   if(detail?.policyId){const policy=state.policies.find(p=>p.id===detail.policyId);if(policy)setInput(`ช่วยอธิบายแผน ${getPlan(policy.planId)?.name} สำหรับกรมธรรม์ตัวอย่าง ${policy.policyNumber} (เป็นข้อมูลเดโม ยังไม่มีข้อมูลเบี้ยกรมธรรม์จริง)`);}
  };
  window.addEventListener("insurance-chat",fn);return()=>window.removeEventListener("insurance-chat",fn);
 },[state.policies,state.profile,state.selection,state.interestedPlanIds,setProfile,setSelection]);
 ...
 function completeDiscovery(persona:DiscoveryPersona){
  if(lock.current)return;
  const next=startDiscoveryState(state,persona);
  flushSync(()=>startDiscovery(persona));
  setName(persona.nickname);setNeeds(next.profile.goals.join("\n"));setQuestions("");setEditedByUser(false);setInput("");setConsent(false);setSubmitted("");setSummaryErrors({});setHandoff(false);setStep(null);
  contextRef.current=JSON.stringify([next.profile,next.selection]);
  if(persona.journey==="browse"){window.location.assign(`/browse?category=${persona.category}`);return;}
  window.history.replaceState(window.history.state,"","/chat");
  void send(personaOpening(persona),{profile:next.profile,selection:next.selection,persona});
 }
 ...
 async function send(content=input,nextContext?:{profile:CustomerProfile;selection:string[];persona?:DiscoveryPersona}){
  const continuation=chatContinuation(state,nextContext);
  const text=content.trim();if(!text||lock.current)return;if(!continuation.resetHistory&&state.messages.length>CHAT_HISTORY_LIMIT-2){setError("บทสนทนานี้ครบ 200 ข้อความแล้ว ประวัติยังอยู่ เริ่มการค้นหาใหม่เพื่อคุยต่อ");return;}if(text.length>2000){setError("ข้อความยาวเกิน 2,000 ตัวอักษร");return;}
  ...
  const persona=nextContext?.persona??state.persona;
  const discoveryIntent=persona?.category===profile.preferredCategory?(persona.journey==="ask"?"ask":persona.journey==="compare"&&(Boolean(nextContext?.persona)||!state.comparedPlanIds.length)?"compare":undefined):undefined;
  const lastShownPlanIds=continuation.lastShownPlanIds;
  const messages=[...continuation.messages,{role:"user" as const,content:text}];
  try{
   const response=await fetch("/api/chat",{method:"POST",headers:{"content-type":"application/json"},body:chatRequestBody(messages,{category:profile.preferredCategory,selectedPlanIds:selection,...(lastShownPlanIds?.length?{lastShownPlanIds}:{}),budgetTHB:profile.budgetTHB,goals:profile.goals,...(persona?{persona}:{}),...(discoveryIntent?{discoveryIntent,priorityKeys:persona?personaFieldKeys(persona):[]}:{}),...(continuation.customerFacts?{customerFacts:continuation.customerFacts}:{})})});
   ...
   const comparisonCard=data.cards.find(card=>card.type==="comparison");if(comparisonCard?.type==="comparison")recordComparison(comparisonCard.planIds);
 ...
 const modeControl=<span className="chat-mode-control">ChatGPT · Live</span>;
 if(fullPage&&step)return <DiscoveryOnboarding initialPersona={state.persona} onComplete={completeDiscovery} onCancel={state.messages.length?()=>{setStep(null);window.history.replaceState(window.history.state,"","/chat");}:undefined}/>;
 const workspace=<>
  <header className="chat-header"><div className="chat-header-title">{fullPage&&<Link href="/" className="chat-home-link" aria-label="กลับหน้าแรก"><ArrowLeft size={22} aria-hidden="true"/></Link>}<div>{fullPage?<h1>คุยเรื่องประกัน</h1>:<Dialog.Title>คุยเรื่องประกัน</Dialog.Title>}{fullPage?<p>ถามเรื่องแผนและความคุ้มครอง</p>:<Dialog.Description>ส่งข้อความไป OpenAI</Dialog.Description>}</div></div>
   <div className="chat-window-actions">{fullPage?<><Link href="/browse" className="chat-browse-link">ดูแผนทั้งหมด<ArrowUpRight size={16} aria-hidden="true"/></Link><button className="chat-restart" disabled={busy} onClick={()=>{setStep("persona");setHandoff(false);setConsent(false);}}>เริ่มเลือกใหม่</button></>:<><Link href={handoff?"/chat?handoff=1":"/chat"} aria-label="เปิดแชตเต็มหน้า" title="เปิดแชตเต็มหน้า"><ArrowUpRight size={20}/></Link><button type="button" aria-label={expanded?"ใช้หน้าต่างแชตขนาดเล็ก":"ขยายหน้าต่างแชต"} onClick={()=>setExpanded(!expanded)}>{expanded?<Shrink size={18}/>:<Expand size={18}/>}</button><button type="button" aria-label="ย่อแชตไว้ด้านล่าง" onClick={()=>{setMinimized(true);setOpen(false);}}><Minus size={20}/></button><Dialog.Close asChild><button aria-label="ปิด AI Chat" onClick={()=>setMinimized(false)}><X size={20}/></button></Dialog.Close></>}</div>
  </header>
  <div className="chat-scroll" ref={scrollArea}><div className="chat-reading-column"><div className="chat-mode-line"><p>ข้อความส่งไป OpenAI</p>{modeControl}</div><p className="sr-only" role="status" aria-live="polite" aria-atomic="true">{announcement}</p>
   {state.assistantOverview&&<details className="chat-analysis"><summary>สรุปข้อมูลจากบทสนทนา</summary><AssistantOverviewView overview={state.assistantOverview}/></details>}
   {/* Local generated artwork uses the existing static asset pipeline. */}
   {/* eslint-disable-next-line @next/next/no-img-element */}
   {fullPage&&state.persona&&<div className="chat-persona-context"><img src={ageBands.find(age=>age.id===state.persona?.ageBand)?.image} alt="" width={56} height={56}/><div><strong>{state.persona.nickname}</strong><span>{ageBands.find(age=>age.id===state.persona?.ageBand)?.label} · {state.profile.goals.join(" · ")}</span></div></div>}
   {fullPage&&state.profile.preferredCategory&&<div className="chat-context"><span>ประกัน{labels[state.profile.preferredCategory]}</span><span>{state.profile.budgetTHB===null?budgetLabel:"งบ "+state.profile.budgetTHB.toLocaleString("th-TH")+" บาท/ปี"}</span></div>}
   <div className="chat-messages">{state.messages.map((message,i)=><div key={i} ref={i===latestAssistant?lastReply:undefined} className={"chat-message "+(message.role==="user"?"is-user":"is-assistant")}><p className={"chat-bubble "+(message.role==="user"?"is-user":"")}>{message.role==="assistant"&&<small>{message.mode==="live"?"ChatGPT · Live":"บทสนทนาตัวอย่าง"}</small>}{message.content}</p>{message.role==="assistant"&&message.cards?.map((card,j)=><ChatCardView key={j} card={card} selection={state.selection} disabled={busy} current={i===latestAssistant&&!onboarding} onAction={cardAction} onCategory={chooseCategory} onBudget={chooseBudget}/>)}</div>)}
    {!fullPage&&(onboarding||state.messages.length===0)&&!busy&&<ChatCardView card={{type:"question",kind:"category",category:null,prompt:"อยากเริ่มจากประกันแบบไหน?"}} selection={state.selection} disabled={busy} current onAction={cardAction} onCategory={chooseCategory} onBudget={chooseBudget}/>} {pendingText&&<p className="chat-bubble is-user">{pendingText}</p>}{busy&&<div className="chat-answer-loading" role="status"><span>กำลังหาคำตอบ…</span><div aria-hidden="true"><i/><i/><i/></div></div>}<div ref={bottom}/></div>
    <div ref={summaryPanel} tabIndex={-1} className="chat-summary-anchor">{submitted&&<section className="p-4 bg-green-50 rounded-xl mt-4" role="status"><h3>{existingLead?"มีคำขอที่รอดำเนินการอยู่แล้ว":"ส่งคำขอแล้ว"}</h3><p className="text-sm">รอผู้เชี่ยวชาญรับเรื่อง · ข้อมูลตัวอย่าง</p><a href={`/broker/leads/${submitted}`} className="text-link">ดูคำขอผู้เชี่ยวชาญ ›</a><a href="/account" className="text-link block">ดูสถานะ ›</a></section>}{handoff&&<section className="summary-preview">{state.assistantOverview&&<AssistantOverviewView overview={state.assistantOverview}/>}<h3 className="font-semibold">ตรวจสรุปก่อนส่ง</h3><p className="text-xs mt-2 text-[#6e6e73]">ข้อมูลจาก {lastMode === "live" ? "Live AI" : "ข้อมูลตัวอย่าง"}</p><p className="text-sm mt-2">หมวด {state.profile.preferredCategory?labels[state.profile.preferredCategory]:"ยังไม่ได้ระบุ"} · งบ {state.profile.budgetTHB!==null?`${state.profile.budgetTHB.toLocaleString("th-TH")} บาท/ปี`:budgetLabel}</p><div className="my-3 text-sm"><h4 className="font-semibold">แผนที่เปรียบเทียบแล้ว</h4>{state.comparedPlanIds.filter(id=>getPlan(id)?.category===state.profile.preferredCategory).length?<ul>{state.comparedPlanIds.filter(id=>getPlan(id)?.category===state.profile.preferredCategory).map(id=><li key={id}>{getPlan(id)?.name}</li>)}</ul>:<p>ยังไม่ได้เปรียบเทียบ</p>}</div><fieldset className="my-4" aria-describedby={summaryErrors.plans?"summary-plans-error":undefined}><legend className="font-semibold text-sm">แผนที่สนใจ *</legend>{[...new Set([...state.selection,...state.interestedPlanIds])].filter(id=>getPlan(id)?.category===state.profile.preferredCategory).map(id=><label key={id} className="flex items-center gap-3 min-h-11 text-sm"><input type="checkbox" checked={state.interestedPlanIds.includes(id)} onChange={e=>{setInterestedPlans(e.target.checked?[...state.interestedPlanIds,id]:state.interestedPlanIds.filter(p=>p!==id));setEditedByUser(true);}}/>{getPlan(id)?.name}</label>)}{summaryErrors.plans&&<p id="summary-plans-error" className="text-sm text-red-700" role="alert">{summaryErrors.plans}</p>}{!state.selection.length&&!state.interestedPlanIds.length&&<a href="/browse" className="text-link">เลือกแผนที่สนใจก่อน ›</a>}</fieldset><label>ชื่อสำหรับเดโม<input className="field" value={name} maxLength={60} aria-required="true" aria-invalid={Boolean(summaryErrors.name)} aria-describedby={summaryErrors.name?"summary-name-error":undefined} onChange={e=>{setName(e.target.value);setEditedByUser(true);}}/>{summaryErrors.name&&<span id="summary-name-error" className="text-sm text-red-700" role="alert">{summaryErrors.name}</span>}</label><label>สิ่งที่อยากให้ช่วย<textarea className="field" aria-label="สิ่งที่อยากให้ช่วย" value={needs} maxLength={1004} aria-invalid={Boolean(summaryErrors.needs)} aria-describedby="summary-needs-hint" onChange={e=>{setNeeds(e.target.value);setEditedByUser(true);}}/><span id="summary-needs-hint" className={summaryErrors.needs?"text-xs text-red-700":"text-xs text-[#6e6e73]"}>{summaryErrors.needs??"บรรทัดละข้อ · สูงสุด 5 ข้อ × 200 ตัวอักษร"}</span></label><label>คำถามถึงผู้เชี่ยวชาญ<textarea className="field" aria-label="คำถามถึงผู้เชี่ยวชาญ" value={questions} maxLength={1004} aria-invalid={Boolean(summaryErrors.questions)} aria-describedby="summary-questions-hint" onChange={e=>{setQuestions(e.target.value);setEditedByUser(true);}}/><span id="summary-questions-hint" className={summaryErrors.questions?"text-xs text-red-700":"text-xs text-[#6e6e73]"}>{summaryErrors.questions??"บรรทัดละข้อ · สูงสุด 5 ข้อ × 200 ตัวอักษร"}</span></label><label>ช่วงเวลาสะดวก<select className="field" value={state.profile.contactWindow} onChange={e=>{setProfile({...state.profile,contactWindow:e.target.value as typeof state.profile.contactWindow});setEditedByUser(true);}}><option value="morning">เช้า</option><option value="afternoon">บ่าย</option><option value="evening">เย็น</option></select></label><label className="flex gap-3 items-center min-h-11"><input type="checkbox" checked={consent} aria-required="true" aria-invalid={Boolean(summaryErrors.consent)} aria-describedby={summaryErrors.consent?"summary-consent-error":undefined} onChange={e=>setConsent(e.target.checked)}/>ยินยอมส่งข้อมูลตัวอย่างให้ผู้เชี่ยวชาญ</label>{summaryErrors.consent&&<p id="summary-consent-error" className="text-sm text-red-700" role="alert">{summaryErrors.consent}</p>}<div className="flex gap-2"><button className="action-primary" onClick={submit}>ยืนยันส่งคำขอ</button><button className="action-secondary" onClick={()=>{setHandoff(false);setConsent(false);setSummaryErrors({});}}>ยกเลิก</button></div></section>}</div>

  </div></div><footer className="chat-composer"><div className="chat-composer-inner">{error&&<p className="chat-card-error" role="alert">{error}</p>}<form onSubmit={event=>{event.preventDefault();void send();}}><input ref={composer} aria-label="ข้อความถึง AI" className="field" value={input} maxLength={2000} onChange={event=>setInput(event.target.value)} placeholder="อยากรู้อะไร ถามได้เลย" disabled={busy}/><button aria-label="ส่งข้อความ" className="action-primary" disabled={busy||!input.trim()}><Send size={20} aria-hidden="true"/></button></form><p>ตรวจเงื่อนไขของแผนก่อนตัดสินใจ</p></div></footer>{confirmationDialog}
 </>;
