...
export function BrokerChat({ lead, draft, onDraft, onUpdate }: { lead: Lead; draft: string; onDraft: (value: string) => void; onUpdate: (lead: Lead) => void }) {
  ...
  return <div className="broker-chat chat-workspace">
    <div className="broker-conversation-meta"><p>ประวัติแชตทั้งหมด</p><button type="button" onClick={()=>{log.current?.scrollTo({top:log.current.scrollHeight});log.current?.focus({preventScroll:true});}}>ข้อความล่าสุด ↓</button></div>
