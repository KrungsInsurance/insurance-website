...
export function SiteNav() {
 ...
 return <header className="site-nav"><nav aria-label={broker?"เมนูผู้เชี่ยวชาญ":"เมนูหลัก"}><Link className="desktop-brand" href={broker?"/broker":"/"}>◒ krungsInsurance{broker?" · Broker":""}</Link><div className="desktop-nav">{items}</div><Link className="mobile-brand" href={broker?"/broker":"/"}>{broker?"krungsInsurance · Broker":"krungsInsurance"}</Link><details className="mobile-nav" onKeyDown={event => { if(event.key === "Escape") { event.currentTarget.open = false; event.currentTarget.querySelector("summary")?.focus(); } }}><summary>เมนู</summary><div>{items}</div></details></nav></header>;
}
